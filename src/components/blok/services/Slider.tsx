...
export default function Slider({ blok }: { blok: SliderBlok }) {
  ...
  const currentFeature =
    blok?.feature && blok.feature.length > 0 ? blok.feature[0] : null;
  ...
  return (
    <section
      ...
      <div className="w-full max-w-[90%]  bg-white rounded-2xl sm:rounded-3xl shadow-xl hover:shadow-2xl overflow-hidden transition-all duration-500 ease-in-out hover:scale-[1.01]">
        ...
        <div
          className={`grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 xl:gap-16 p-6 sm:p-8 md:p-10 lg:p-12 xl:p-16 min-h-[500px] sm:min-h-[600px] lg:min-h-[700px] ${
            imageOnRight ? "lg:grid-flow-col-dense" : ""
          }`}>
          {/* Content Area - Centered text content */}
          <div
            className={`flex flex-col justify-center items-center text-center lg:text-left lg:items-start space-y-6 sm:space-y-8 md:space-y-10 h-full ${
              imageOnRight ? "lg:col-start-2" : ""
            }`}>
            {/* H2 Heading - Centered with responsive typography */}
            <h2
              data-blok-field="feature"
                ...
                <div className="space-y-3 sm:space-y-4">
                  {(currentFeature?.description || "")
                    ...
                    .map((paragraph, index) => (
                      <p
                        key={index}
                        className="text-sm sm:text-base md:text-lg leading-relaxed transition-all duration-300 line-clamp-4">
                        {paragraph}
                      </p>
                    ))}
                </div>
            ...
            {/* CTA Button - Centered */}
            <div
              className="pt-2 sm:pt-4 flex-shrink-0"
              data-blok-field="feature">
